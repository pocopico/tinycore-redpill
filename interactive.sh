...
function buildmenu() {
	...
	case $menuitem in
	build)
		getvars $TARGET_PLATFORM-$TARGET_VERSION-$TARGET_REVISION
		checkinternet
		getlatestrploader
		gitdownload
...
function extmenu() {
	...
	addlist)
		...
		for ext in $(cat $OUTPUT); do
			dialog --msgbox "$(/home/tc/redpill-load/ext-manager.sh add https://raw.githubusercontent.com/pocopico/rp-ext/master/$ext/rpext-index.json 2>&1 | sed "s,\x1B\[[0-9;]*[a-zA-Z],,g")" 30 90
			sleep 1
		done
		;;
	...
	remove)
		LIST=$(
			let num=0
			for item in $(find /home/tc/redpill-load/custom/extensions/* -type d | awk -F\/ '{print $7}'); do
				let num=$num+1
				echo "$item $num off"
			done
		)
...
function mainmenu() {
	...
	while true; do
		...
		build) buildmenu ;;
		ext) extmenu ;;
		download) dialog --msgbox "$(
			getvars $TARGET_PLATFORM-$TARGET_VERSION-$TARGET_REVISION
			checkinternet
			gitdownload
		)" 30 90 ;;
		listmods)
			msgbox "$(listmodules) 2>&1" 30 90
			echo "$extensionslist"
			;;
...
dialog --backtitle "RedPill Loader" --infobox "Welcome to Tinycore RedPill Loader Interactive" 3 45
sleep 1
